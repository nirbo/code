...
use code_core::RateLimitSwitchState;
...
use code_core::switch_active_account_on_rate_limit;
...
fn run_auto_loop(
    ...
    let mut pending_conversation = Some(Arc::<[ResponseItem]>::from(filter_popular_commands(
                ...
                Err(failure) => {
                    ...
                    if let Some(recoverable) = classify_recoverable_decision_error(&error) {
                        ...
                        if consecutive_decision_failures <= MAX_DECISION_RECOVERY_ATTEMPTS {
                            ...
                            {
                                let retry_vec = retry_conversation
                                    .get_or_insert_with(|| conv.as_ref().to_vec());
...
fn run_git_command<const N: usize>(args: [&str; N]) -> Option<String> {
    ...
    if args
        .iter()
        .any(|arg| matches!(*arg, "pull" | "checkout" | "merge" | "apply"))
    {
