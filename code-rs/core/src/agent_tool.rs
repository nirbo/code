...
impl AgentManager {
    ...
    fn append_agent_log(&self, log_tag: &str, line: &str) {
        let Some(root) = &self.debug_log_root else {
            return;
        };
...
async fn execute_model_with_permissions(
    ...
    let debug_subagent =
        debug_subagents_enabled() && matches!(source_kind, Some(AgentSourceKind::AutoReview));
