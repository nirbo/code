...
use super::explore::ExploreAggregationCell;
use super::explore::explore_lines_from_record;
...
pub(crate) fn cell_from_record(record: &HistoryRecord, cfg: &Config) -> Box<dyn HistoryCell> {
    match record {
        ...
        HistoryRecord::UpgradeNotice(state) => {
            Box::new(UpgradeNoticeCell::from_state(state.clone()))
        }
        HistoryRecord::Reasoning(state) => {
            Box::new(CollapsibleReasoningCell::from_state(state.clone()))
        }
        ...
        HistoryRecord::AssistantStream(state) => Box::new(StreamingContentCell::from_state(
            state.clone(),
            cfg.file_opener,
            cfg.cwd.clone(),
        )),
...
pub(crate) fn record_from_cell(cell: &dyn HistoryCell) -> Option<HistoryRecord> {
    ...
    if let Some(background) = cell.as_any().downcast_ref::<BackgroundEventCell>() {
    ...
    if let Some(explore) = cell.as_any().downcast_ref::<ExploreAggregationCell>() {
    ...
    if let Some(running_tool) = cell.as_any().downcast_ref::<RunningToolCallCell>() {
    ...
    if let Some(reasoning) = cell.as_any().downcast_ref::<CollapsibleReasoningCell>() {
    ...
    if let Some(stream) = cell.as_any().downcast_ref::<StreamingContentCell>() {
        return Some(HistoryRecord::AssistantStream(stream.state().clone()));
    }
    if let Some(assistant) = cell.as_any().downcast_ref::<AssistantMarkdownCell>() {
    ...
    if let Some(rate_limits) = cell.as_any().downcast_ref::<RateLimitsCell>() {
