            ...
            match event {
                ...
                AppEvent::OpenExternalEditor { initial } => {
                    ...
                    let editor_result = match tui::restore() {
                        ...
                        Err(err) => Err(external_editor::ExternalEditorError::LaunchFailed(
                            format!("Failed to reset terminal: {err}",),
                        )),
                    };
                ...
                AppEvent::DispatchCommand(command, command_text) => {
                    ...
                    match command {
                        ...
                        SlashCommand::Logout => {
                            if let Err(e) = code_login::logout(&self.config.code_home) {
                ...
                AppEvent::UpdateTheme(new_theme) => {
                    ...
                    if matches!(new_theme, code_core::config_types::ThemeName::Custom) {
                        ...
                        } else if let Ok(cfg) = code_core::config::Config::load_with_cli_overrides(
                            vec![],
